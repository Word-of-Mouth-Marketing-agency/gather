...
export default function AddressesPage() {
  ...
  return (
    <>
      <PageTitleSection title="My Addresses" />
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading ? (
          <div className="text-center py-16 text-[#7a6247]">Loading addresses...</div>
        ) : addresses.length === 0 && !showForm ? (
          <div className="text-center py-16">
            <div className="text-4xl mb-4">📍</div>
            <h2 className="text-xl font-black text-[#171717]">No addresses saved</h2>
            <p className="text-sm text-[#7a6247] mt-2">Add a delivery address for faster checkout.</p>
              ...
              <div key={addr.id} className="gather-section p-5 rounded-3xl flex items-start justify-between gap-4">
                <div className="text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    {addr.isDefault && <span className="text-[10px] bg-[#ff7a1a]/10 text-[#ff7a1a] font-semibold px-2 py-0.5 rounded-full">Default</span>}
                  </div>
                ...
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => openEdit(addr)} className="text-xs text-[#ff7a1a] font-semibold hover:underline">Edit</button>
                  <button onClick={() => handleDelete(addr.id)} className="text-xs text-red-500 font-semibold hover:underline">Delete</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-5" role="dialog" aria-modal="true">
            ...
            <div className="relative z-10 w-full max-w-md rounded-3xl bg-white shadow-xl p-8">
              <h3 className="text-lg font-black text-[#171717] mb-4">{editingId ? 'Edit Address' : 'Add Address'}</h3>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" value={form.street} onChange={(e) => setForm({...form, street: e.target.value})} required
                    className="w-full min-h-[45px] rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:outline-none focus:border-[#ff7a1a] focus:ring-2 focus:ring-[#ff7a1a]/20 transition-colors"
                    placeholder="123 El Nasr Street" />
                </div>
                <div className="space-y-1.5">
                  <label className="block text-sm font-semibold text-gray-700">Apartment (optional)</label>
                  <input type="text" value={form.apartment} onChange={(e) => setForm({...form, apartment: e.target.value})}
                    className="w-full min-h-[45px] rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:outline-none focus:border-[#ff7a1a] focus:ring-2 focus:ring-[#ff7a1a]/20 transition-colors"
                    placeholder="Apt 4B" />
                </div>
                <div className="space-y-1.5">
                  <label className="block text-sm font-semibold text-gray-700">Phone for this address</label>
