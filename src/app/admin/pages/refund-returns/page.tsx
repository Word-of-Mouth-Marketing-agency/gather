...
export default function AdminRefundReturnsEditorPage() {
  ...
  useEffect(() => {
    fetch('/api/pages/refund-returns')
      .then((r) => r.json())
      .then((data) => {
        setContent(data)
        setLoading(false)
  ...
  const save = useCallback(async () => {
    ...
    setSaveStatus('idle')
    try {
      const res = await fetch('/api/pages/refund-returns', {
        method: 'PUT',
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-black text-gray-900">Refund & Returns Editor</h1>
        </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
          <p className="text-red-600 font-semibold">Failed to load refund & returns content</p>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6 pb-24">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-900">Refund & Returns Editor</h1>
            <p className="text-xs text-gray-400 mt-0.5">Manage refund and returns policy page content</p>
          </div>
        </div>
        <button
          onClick={save}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-50 bg-gray-50/50">
          ...
          <p className="text-xs text-gray-400 mt-0.5">General settings for the Refund & Returns Policy page.</p>
        </div>
